...
class Enhance_RestoreFormerPPlus(BaseProcessor):
    ...
    def Initialize(self, plugin_options:dict):
        ...
        if self.model_restoreformerpplus is None:
            ...
            self.model_restoreformerpplus = create_inference_session(model_path, self.processorname)
    ...
    def Run(self, source_faceset: FaceSet, target_face: Face, temp_frame: Frame) -> Frame:
        ...
        batch_input = self._get_batch_buffer("restoreformer_input_single", (1, 3, 512, 512), np.float32)
    ...
    def RunBatch(self, source_facesets, target_faces, temp_frames, batch_size=1):
        ...
        for batch_start in range(0, len(temp_frames), effective_batch_size):
            ...
            if not current_frames:
                continue
            batch_input = self._get_batch_buffer("restoreformer_input", (effective_batch_size, 3, 512, 512), np.float32)
            ...
            try:
                batch_outputs = self.model_restoreformerpplus.run(
                    None,
                    {getattr(self, "input_name", self.model_inputs[0].name): active_batch},
                )[0]
