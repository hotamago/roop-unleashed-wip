    ...
    with gr.Tab("Settings"):
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                detect_single_batch_workers = gr.Slider(1, 32, value=getattr(roop.config.globals.CFG, "detect_single_batch_workers", 1), label="Detect Single-Batch Workers", info='Parallel detector worker sessions used when the selected detector model only supports batch=1.', step=1.0, interactive=True)
                single_batch_workers = gr.Slider(1, 32, value=roop.config.globals.CFG.single_batch_workers, label="Single-Batch Workers", info='Parallel worker sessions for models limited to batch=1. The runtime will try to allocate exactly the requested worker count.', step=1.0, interactive=True)
